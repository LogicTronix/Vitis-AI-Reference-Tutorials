...
def ResNet50(image_channels=3, num_classes=3):
...
def main():
  ...
  model.load_state_dict(torch.load("ResNet50.pth", map_location=torch.device(device)))
  ...
  if args.mode == 'train':
    ...
    torch.save(qat_model.state_dict(), "ResNet50_QAT_trained.pth")
    ...
    quantized_model.load_state_dict(torch.load("ResNet50_QAT_trained.pth", map_location=torch.device(device)))
